...
import { exec, parseJSONFileIfExists, extensionConfiguration } from "../utils";
import {
  Targets,
  Dependencies,
  BuildOptions,
...
export async function getMesonTargets(build: string) {
  ...
  if (!parsed) {
    const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} introspect --targets`, {
      cwd: build
    });
...
export async function getMesonBuildOptions(build: string) {
  ...
  const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} introspect --buildoptions`, {
    cwd: build
  });
...
export async function getMesonProjectInfo(build: string) {
  ...
  if (parsed) return parsed;
  const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} introspect --project-info`, {
    cwd: build
  });
...
export async function getMesonDependencies(build: string) {
  ...
  const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} introspect --dependencies`, {
    cwd: build
  });
...
export async function getMesonTests(build: string) {
  ...
  if (parsed) return parsed;
  const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} introspect --tests`, { cwd: build });
...
export async function getMesonBenchmarks(build: string) {
  ...
  const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} introspect --benchmarks`, {
    cwd: build
  });
  return JSON.parse(stdout) as Tests;
}
...
export async function getMesonVersion(): Promise<[number, number, number]> {
  const { stdout } = await exec(`${extensionConfiguration("mesonExecutable")} --version`, {});
